...
public class BlockStateCacheImpl {
    ...
    // MODIFIED for porting: upstream reached the private BlockStateBase.cache field through a memoized MethodHandle
    // plus a loader-specific field name lookup, because Mixin cannot access fields of private inner classes. In a
    // source-level port the field is simply exposed by BlockStateBase itself.
    @Nullable
    private static BlockStateCacheAccess getCache(BlockStateBase state) {
        return state.ferritecore_getCache();
    }

    // Calls before the cache for <code>state</code> is (re-)populated
    public static void deduplicateCachePre(BlockStateBase state) {
        LAST_CACHE.set(getCache(state));
    }

    // Calls after the cache for <code>state</code> is (re-)populated
    public static void deduplicateCachePost(BlockStateBase state) {
        BlockStateCacheAccess newCache = getCache(state);
        if (newCache != null) {
