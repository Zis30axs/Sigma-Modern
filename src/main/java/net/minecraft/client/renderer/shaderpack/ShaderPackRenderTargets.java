...
final class ShaderPackRenderTargets {
    ...
    void prepare(final CommandEncoder encoder, final int width, final int height, final Collection<Integer> requiredTargets) {
        ...
        for (int target : requiredTargets) {
            ...
            if (target == 0 || !this.preparedThisFrame.add(target)) {
                continue;
            }

            TextureTarget renderTarget = this.ensureOwnedTarget(this.mainTargets, target, "main");
            GpuTexture texture = Objects.requireNonNull(renderTarget.getColorTexture(), "colortex" + target + " main texture is unavailable");
    ...
    void copyCurrentColor0ToPrimary(final CommandEncoder encoder, final RenderTarget primaryTarget) {
        if (!this.flipped[0]) {
            return;
        }
