...
public class TraversableTree extends Tree {
    ...
    static boolean cylindricalDistanceTest(float dx, float dy, float dz, float distanceLimit) {
        // MODIFIED for porting: was sodium-extra's fog MixinTraversableTree#sodiumExtra$testExpandedCylindricalDistance
        // (@Inject HEAD, cancellable)
        if (me.flashyreese.mods.sodiumextra.client.config.SodiumExtraFeatures.FOG && me.flashyreese.mods.sodiumextra.client.fog.FogDistanceHelper.isExpandedCylindricalCullDistance(distanceLimit)) {
            return me.flashyreese.mods.sodiumextra.client.fog.FogDistanceHelper.testExpandedCylindricalCullDistance(dx * dx + dz * dz, dy, distanceLimit);
        }
