...
public class OcclusionCuller {
    ...
    private void processQueue(ReadQueue<RenderSection> readQueue,
        ...
        while ((section = readQueue.dequeue()) != null) {
            ...
            // MODIFIED for porting: was sodium-extra's fog MixinOcclusionCuller
            // #sodiumExtra$allowExpandedCylindricalVerticalTraversal (@ModifyArgs on this visitNeighbors call)
            if (me.flashyreese.mods.sodiumextra.client.config.SodiumExtraFeatures.FOG
                && (me.flashyreese.mods.sodiumextra.client.fog.FogDistanceHelper.isExpandedCylindricalCullDistance(this.searchDistanceRegular)
                    || me.flashyreese.mods.sodiumextra.client.fog.FogDistanceHelper.isExpandedCylindricalCullDistance(this.searchDistanceLocal))) {
                int verticalDirections = sodiumExtra$getOutwardVerticalDirections(this.inBoundsOrigin, section);
                outgoingWide |= verticalDirections;
                outgoingRegular |= verticalDirections;
                outgoingLocal |= verticalDirections;
            }
    ...
    // MODIFIED for porting: was sodium-extra's fog MixinOcclusionCuller @Unique constants and helper
    ...
    private static boolean testDistance(float xzThreshold, float yThreshold, float maxDistance) {
        // MODIFIED for porting: was sodium-extra's fog MixinOcclusionCuller#sodiumExtra$testExpandedCylindricalDistance
        // (@Inject HEAD, cancellable)
        if (me.flashyreese.mods.sodiumextra.client.config.SodiumExtraFeatures.FOG && me.flashyreese.mods.sodiumextra.client.fog.FogDistanceHelper.isExpandedCylindricalCullDistance(maxDistance)) {
            return me.flashyreese.mods.sodiumextra.client.fog.FogDistanceHelper.testExpandedCylindricalCullDistance(xzThreshold, yThreshold, maxDistance);
        }
