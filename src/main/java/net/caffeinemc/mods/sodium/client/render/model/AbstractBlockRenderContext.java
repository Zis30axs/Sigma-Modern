...
public abstract class AbstractBlockRenderContext extends AbstractRenderContext {
    ...
    public void bufferDefaultModel(BlockStateModelPart part, Predicate<Direction> cullTest, Consumer<MutableQuadViewImpl> emitter) {
        ...
        for (int i = 0; i <= ModelHelper.NULL_FACE_ID; i++) {
            ...
            // MODIFIED for porting: was iris's compat.sodium MixinAbstractBlockRenderContext#checkDirectionNeo (@Inject at
            // the INVOKE of PlatformModelAccess#getQuads, with @Local Direction) - a mod can report a different "appearance"
            // block state for a model part, which the pack should see instead of the real one.
            if (net.irisshaders.iris.mixin.IrisMixinPlugin.isEnabled()
                && this instanceof net.irisshaders.iris.vertices.sodium.terrain.VertexEncoderInterface irisEncoder
                && net.irisshaders.iris.shaderpack.materialmap.WorldRenderingSettings.INSTANCE.getBlockStateIds() != null
                && cullFace != null) {
                net.minecraft.world.level.block.state.BlockState irisOverride = net.irisshaders.iris.compat.general.IrisModSupport.INSTANCE
                    .getModelPartState(part);
                if (irisOverride != null) {
                    irisEncoder.overrideBlock(net.irisshaders.iris.shaderpack.materialmap.WorldRenderingSettings.INSTANCE.getBlockStateIds().getInt(irisOverride));
                }
            }
        ...
        editorQuad.clear();
        // MODIFIED for porting: was iris's compat.sodium MixinAbstractBlockRenderContext's second #checkDirectionNeo
        // (@Inject TAIL)
        if (net.irisshaders.iris.mixin.IrisMixinPlugin.isEnabled()
            && this instanceof net.irisshaders.iris.vertices.sodium.terrain.VertexEncoderInterface irisEncoder
            && net.irisshaders.iris.shaderpack.materialmap.WorldRenderingSettings.INSTANCE.getBlockStateIds() != null) {
            irisEncoder.restoreBlock();
        }
    }
}
