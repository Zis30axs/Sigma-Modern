...
public class VideoSettingsScreen extends Screen implements ScreenPromptable, ScrollableTooltip.TooltipParent {
    ...
    public static int renderIconWithSpacing(GuiGraphicsExtractor graphics, Identifier icon, int color, boolean iconMonochrome, int x, int y, int height, int margin) {
        ...
        // MODIFIED for porting: was iris's compat.sodium MixinSodiumGameOptions#iris$makeColor (@WrapOperation around
        // GuiGraphicsExtractor#blit, require = 0) - iris's own config icon is drawn in colour, and untinted, once a shader pack
        // is loaded.
        if (net.irisshaders.iris.mixin.IrisMixinPlugin.isEnabled() && icon.getNamespace().equals("iris") && net.irisshaders.iris.Iris.getCurrentPack().isPresent()) {
            graphics
                .blit(
                    RenderPipelines.GUI_TEXTURED,
                    net.irisshaders.iris.compat.sodium.config.IrisConfig.COLOR,
                    x,
                    y,
                    0,
                    0,
