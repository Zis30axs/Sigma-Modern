...
// MODIFIED for porting: iris's compat.sodium EnumOptionBuilderImplAccessor declares an @Accessor("enumClass") named
// getEnumClass(), but this class already declares a method with exactly that name and signature (below), which resolves the
// value through the parent option instead of reading the field raw. Nothing in iris ever calls the accessor - it only appears
// in its mixin config - so the interface is deliberately not attached here rather than changing the meaning of the existing
// method or inventing a second one.
