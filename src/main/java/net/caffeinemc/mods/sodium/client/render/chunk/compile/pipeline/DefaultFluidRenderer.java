 ...
 */
public class DefaultFluidRenderer implements net.irisshaders.iris.vertices.sodium.terrain.VertexEncoderInterface { // MODIFIED for porting: iris compat.sodium MixinDefaultFluidRenderer
    /**
     * MODIFIED for porting: iris compat.sodium MixinDefaultFluidRenderer @Unique fields (its VertexEncoderInterface implementation) - the block
    ...
    public void render(LevelSlice level, BlockState blockState, FluidState fluidState, BlockPos blockPos, BlockPos offset, TranslucentGeometryCollector collector, ChunkModelBuilder meshBuilder, Material material, ColorProvider<FluidState> colorProvider, FluidModel sprites) {
        ...
        for (Direction dir : DirectionUtil.HORIZONTAL_DIRECTIONS) {
            ...
            if (this.isFluidSideExposed(level, blockState, this.scratchPos, dir, sideFluidHeight)) {
                ...
                // MODIFIED for porting: was iris's compat.sodium MixinDefaultFluidRenderer#setBrightness (@ModifyArg on the
                // third updateQuad call) - a shader pack can disable vanilla's directional shading.
                float br = net.irisshaders.iris.mixin.IrisMixinPlugin.isEnabled() && net.irisshaders.iris.shaderpack.materialmap.WorldRenderingSettings.INSTANCE.shouldDisableDirectionalShading()
                    ? 1.0F
                    : dir.getAxis() == Direction.Axis.Z ? 0.8F : 0.6F;
    ...
    private void writeQuad(ChunkModelBuilder builder, TranslucentGeometryCollector collector, Material material, BlockPos offset, ModelQuadView quad,
        ...
        for (int i = 0; i < 4; i++) {
            ...
            // MODIFIED for porting: was iris's compat.sodium MixinDefaultFluidRenderer#iris$writeVertex (@Inject at the FIELD
            // write of ChunkVertexEncoder$Vertex.x)
