...
public class SodiumConfigBuilder implements ConfigEntryPoint {
    ...
    public SodiumConfigBuilder() {
        ...
        // Null-tolerant: during early config registration the client instance does not exist yet.
        this.window = minecraft == null ? null : minecraft.getWindow();

        this.vanillaOpts = minecraft == null ? null : minecraft.options;
    ...
    /**
     * MODIFIED for porting: sodium-extra and iris expose their config icons as identifiers that are never backed by a
     * registered texture in this loaderless environment (upstream relies on loader-time registration), so the auto
     * SimpleTexture load path renders the missing-texture checkerboard. Register them directly from the classpath root,
     * exactly like {@link #registerIcon(TextureManager)} does for sodium itself.
     */
    public static void registerPortedModIcons(TextureManager textureManager) {
        final var extraIcon = Identifier.parse("sodium-extra:textures/icon.png");
        final var irisMono = Identifier.fromNamespaceAndPath("iris", "textures/gui/config-icon-mono.png");
        final var irisColor = Identifier.fromNamespaceAndPath("iris", "textures/gui/config-icon.png");
        textureManager.registerAndLoad(extraIcon, new ClasspathLogo(extraIcon, "/sodiumextra-icon.png"));
        textureManager.registerAndLoad(irisMono, new ClasspathLogo(irisMono, "/iris-icon.png"));
        textureManager.registerAndLoad(irisColor, new ClasspathLogo(irisColor, "/iris-icon-color.png"));
    }

    static class ClasspathLogo extends ReloadableTexture {
        private final String classpathPath;

        public ClasspathLogo(Identifier identifier, String classpathPath) {
            super(identifier);
        ...
        public TextureContents loadContents(ResourceManager resourceManager) throws IOException {
            try (InputStream inputStream = SodiumConfigBuilder.class.getResourceAsStream(this.classpathPath)) {
                if (inputStream == null) {
                    throw new java.io.FileNotFoundException(this.classpathPath);
                }
