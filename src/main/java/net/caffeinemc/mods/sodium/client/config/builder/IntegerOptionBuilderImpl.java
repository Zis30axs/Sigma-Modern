...
class IntegerOptionBuilderImpl extends StatefulOptionBuilderImpl<IntegerOption, Integer> implements IntegerOptionBuilder,
    net.irisshaders.iris.compat.sodium.mixin.IntegerOptionBuilderImplAccessor { // MODIFIED for porting: iris compat.sodium IntegerOptionBuilderImplAccessor
    private DependentValue<? extends SteppedValidator> validatorProvider;
    private ControlValueFormatter valueFormatter;

    // MODIFIED for porting: was iris's compat.sodium IntegerOptionBuilderImplAccessor @Accessor("valueFormatter") (unused in
    // iris, but declared in its mixin config)
    @Override
